                ...
                ->arrayNode('redis')
                    ...
                    ->children()
                        ...
                        ->scalarNode('prefix')
                            ->cannotBeEmpty()
                            ->validate()
                                ->ifTrue(fn ($s) => 1 !== preg_match('/^%[^%]*%$/', (string) $s) && 1 !== preg_match('/^[a-zA-Z_:][a-zA-Z0-9_:]*$/', (string) $s))
                ...
                ->arrayNode('storage')
                    ...
                    ->children()
                        ...
                        ->scalarNode('prefix')
                            ...
                            ->cannotBeEmpty()
                            ->validate()
                                ->ifTrue(fn ($s) => !preg_match('/^%[^%]*%$/', (string) $s) && 1 !== preg_match('/^[a-zA-Z_:][a-zA-Z0-9_:]*$/', (string) $s))
                                ->thenInvalid('Invalid prefix. Make sure it matches the following regex: ^[a-zA-Z_:][a-zA-Z0-9_:]*$')
                            ->end()
                        ->end()
                        ->arrayNode('options')
                ...
                ->arrayNode('labels')
                    ->arrayPrototype()
                        ->children()
                            ->scalarNode('name')
                                ->isRequired()
                                ->info('Name of the label that will appear in the metric')
                                ->cannotBeEmpty()
                            ->end()
                            ->enumNode('type')
                            ...
                            ->scalarNode('value')
                                ->isRequired()
                                ->info('Name of the attribute or header in the Request')
                                ->cannotBeEmpty()
                            ->end()
                        ->end()
                    ->end()
